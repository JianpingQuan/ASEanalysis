...
for(i in 1:4){
  for(j in 1:3){
    st <- stage[i]
    ti <- tissue[j]
    assign(paste0(ti,st), ASE_finder(tissue=ti,stage=st,fc=2,component="AG",writeout=TRUE))
  }
}

Pl40 <- ASE_finder(tissue="Pl",stage="40",fc=2,component="AG",writeout=TRUE)
Pl70 <- ASE_finder(tissue="Pl",stage="70",fc=2,component="AG",writeout=TRUE)
...
write.csv(all, "ASE_N-1FC2_filter.AG2.csv", quote=F)



##------------------------------------------add gene with loose standard-------------------------------------
#----------Generate FC2_filter_final.AG.csv----------
setwd("F:\\RNA_20221101\\02_ModifiedGenome\\D_AGEgeneSelection\\result")

library(dplyr)
tissue <- c("Br", "Li", "Mu","Pl")
stage <- c(40,70,115,168)
dat <- read.csv("ASE_N-1FC2_filter.AG2.csv", header = T, row.names = 1)

needremove <- read.csv("poe_age_both.csv",header=T,row.names=1) %>% filter(.,type=="POE")
...
write.csv(a, "AGE.list.csv", quote = F)

#Brain,Liver,Muscle
for(i in 1:3){
  for(j in 1:4){
    Group <- read.csv(paste0(tissue[i], "_", stage[j], "_AllGenesAG2.csv"), header = T, row.names = 1)
...
write.csv(all_final, "ASE_N-1_filter_final.AG2.csv",quote=F)


##----------Output AGE genes expreesion breed----------
setwd("F:\\RNA_20221101\\02_ModifiedGenome\\D_AGEgeneSelection\\result")
library(dplyr)
dat <- read.csv("ASE_N-1_filter_final.AG2.csv", header = T, row.names = 1)
tissue <- c("Br","Li","Mu","Pl")
stage <- c("40","70","115","168")
...
for(i in 1:4){
  ...
  for(j in 1:n){
    raw <- read.csv(paste0(tissue[i], "_", stage[j], "_AllGenesAG2.csv"), header = T, row.names = 1)
    col_name <- paste0(tissue[i], "_", stage[j])
    index <- col_name == colnames(dat)
    ...
    for(k in 1:nrow(gene_FC)){
      if(gene_FC[k,1]< 0){
        bias <- append(bias,"Lulai")
      }else if(gene_FC[k,1] >0){
        bias <- append(bias,"Duroc")
      }
    }
    gene_FC$bias <- bias
