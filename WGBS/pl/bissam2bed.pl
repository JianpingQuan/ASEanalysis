#! /usr/bin/perl -w
# modify sam2bed.pl to process at the same time the XM tag
# which specifies the methylation status of the read
# ============================================================
...
while (<>) {
	...
	@parse = processCIGAR($thiscigar, $thismd, $line[9], $thisstart, $thisxm);
	...
	print $line[2], "\t", $thisstart, "\t", $thisstart + $blocksum, "\t",
		...
		"\t", $parse[6], ":", join(",", @var), "\t", $#{$parse[0]} + 1, "\t", join(",", @{$parse[1]}), "\t", join(",", @{$parse[0]}), "\n";
  ...
  my $snpcount = 0;
	...
	while (!($cigar =~ m/^$/)) {
      ...
      $snpcount++;
			...
			substr $xm, $prereadblockstart + $prereadblocksize - $readoffset, $1, "";
			$readoffset += $1;
			$prereadblocksize += $1;
			$cigar = substr $cigar, length($1) + 1;
      ...
      substr $xm, $prereadblockstart + $prereadblocksize - $readoffset, 0, "-" x $1;
			$readoffset -= $1;
	...
	# now process the MD tag and the XM tag
	...
	if ($md ne "") {
        ...
        # U     methylated C in Unknown context (CN or CHN)
          ...
          $snpcount++;
                ...
                # when there is mismatch, check if it is methylated or not
                $me = substr $xm, $preend + $pos, length($base);
                ...
                if ($me eq ".") {
                  ...
                  push(@vartype, "MT");
                  push(@varbase, substr($seq, $preend + $pos, length($base))."_".$me);
                  push(@refbase, $base);
                  push(@varpos, $start + $blockstart[$thisint] + $preend + $pos - $cumsum[$thisint]);
									# replace $xm for subsequent id of methylated bases
									substr $xm, $preend + $pos, length($base), "." x length($base);
	...
	# now process the $xm tag to look for MTs that have been 
	# missed, these are presumably the methylated bases
	# ============================================================
	...
	while ($xm =~ m/[^\.]/g) {
		...
		my $matchStart = $-[0];
		my $thisStatus = substr $xm, $matchStart, 1;
		
		for (my $i = $thisint; $i <= $#cumsum; $i++) {

			if ($matchStart >= $cumsum[$i] && $matchStart + 1 < $cumsum[$i] + $blocksize[$i]) {
		...
		push(@vartype, "MT");
		push(@varbase, $thisStatus."_".$thisStatus);
		push(@refbase, substr($seq, $matchStart, 1));
		push(@varpos, $start + $blockstart[$thisint] + $matchStart - $cumsum[$thisint]);
	...
	return(\@blockstart, \@blocksize, \@varpos, \@vartype, \@refbase, \@varbase, $snpcount);
