...
args=`getopt -o "r:,d:,t:,o:,s:,b:" -l "resource:,dir:,tmp:,out:,sample:,bam:" -- "$@"`
...
if [[ -e $bam ]]
then
  >&2 echo "$(date +"%m-%d-%Y-%T"):info: found $bam."
else
  >&2 echo "$(date +"%m-%d-%Y-%T"):error: cannot find $bam."
  exit 1
fi
...
jobID=$(sbatch --export=env=$resource/resource.env,tmp=$tmp/$id/tmp,input=$bam,output=$tmp/$id/rmdup.bam,out=$out/$sample,mem=32G,metric=$out/$sample/rmdup.metric.out --output=$out/$sample/log/markDuplicates.out --error=$out/$sample/log/markDuplicates.err $resource/sbatch/markDuplicates.sbatch | cut -d " " -f 4)
sleep 5m
